...
- Caller-save registers that can be modified during a function call.
  ...
  We follow the Patmos Handbook application binary interface (ABI) in our
  choice of callee- and caller-save registers.
*)

Definition is_callee_save (r: mreg) : bool :=
  match r with
  | R1  | R2  | R3  | R4  | R5  | R6  | R7  | R8  | R9  | R10
  | R11 | R12 | R13 | R14 | R15 | R16 | R17 | R18 | R19 => false
                                                             
  | R20 | R21 | R22 | R23 | R24 | R25 | R26 | R27 | R28 => true
  end.
...
Definition dummy_int_reg   := R19.    (**r Used in [Coloring]. *)
...
(** The functions in this section determine the locations (machine registers
  ...
  implement the conventions form the Patmos Reference Handbook.  *)

(** ** Location of function result *)

(** The result value of a function is passed back to the caller in
  registers [R1] or [R1,R2], depending on the type of the
...
Definition loc_result (s: signature) : rpair mreg :=
  ...
  | None => One R1
  | Some (Tint | Tany32 | Tany64) => One R1
  | Some (Tfloat | Tsingle) => One R1
  | Some (Tlong) => Twolong R1 R2
  end.
...
Proof.
  ...
  Focus 2. split. congruence. split; auto.
Admitted.
...
(** The Patmos LLVM ABI states the following convention for passing arguments
  to a function:

- Arguments are passed in registers when possible.

- Up to six registers (ai: int_param_regs) are used for this purpose.

- For 64-bit arguments, the high part is stored first, then the low part.
...
Fixpoint loc_arguments_rec (va: bool)
  ...
  | (Tfloat | Tany64) as ty :: tys =>
      ...
      then hybrid_arg int_param_regs r ofs ty (loc_arguments_rec va tys)
      else one_arg int_param_regs r ofs ty (loc_arguments_rec va tys)
  end.
...
Proof.
  ...
  { red. decide_goal. }
  assert (E: OKREGS int_param_regs).
  { red. decide_goal. }
